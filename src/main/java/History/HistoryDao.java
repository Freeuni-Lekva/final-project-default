...
public class HistoryDao implements IHistoryDao{
    ...
    public ArrayList <History> getUsersFromHistory(int quiz_id) throws SQLException {
        ...
        ResultSet resultSet = statement.executeQuery("select * from history where quiz_id = " + quiz_id + " order by score desc datediff(minute, start_time, end_time) asc limit 50");
    ...
    public ArrayList <History> getQuizzesFromHistory(int user_id) throws SQLException {
        Statement statement = connection.createStatement();
        ResultSet resultSet = statement.executeQuery("select * from history where user_id = " + user_id + " order by score desc datediff(minute, start_time, end_time) asc limit 50");
        ArrayList <History> arr = getHistoryObjects(resultSet);
        statement.close();
        return arr;
    }
    ...
    public ArrayList <History> getScore(int quiz_id, int user_id) throws SQLException {
        ...
        ResultSet resultSet = statement.executeQuery("select * from history where user_id = " + user_id +
                " and quiz_id = " + quiz_id + " order by score desc datediff(minute, start_time, end_time) asc limit 50");
        ArrayList <History> arr = getHistoryObjects(resultSet);
        statement.close();
        return arr;
    }
    ...
    public ArrayList<History> getAllHistory() throws SQLException {
        Statement statement = connection.createStatement();
        ResultSet resultSet = statement.executeQuery("select * from history order by score desc datediff(minute, start_time, end_time) asc limit 50"); // limit 50
        ArrayList <History> arr = getHistoryObjects(resultSet);
        statement.close();
        return arr;
    }
